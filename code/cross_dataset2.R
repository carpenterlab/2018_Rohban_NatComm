...
Pf.1.mad <- read.and.summarize("mad", "../../CDRP_2nd_moment", feat.list, metadata.cdrp)
Pf.2.mad <- read.and.summarize("mad", "../../repurp_2nd_moment", feat.list, metadata.repurp)
...
c2 <- rbind(cbind(m11, cr.1.2.mad), cbind(t(cr.1.2.mad), m22))
...
cr.4 <- cr.median.mad[1:NROW(Pf.1.mean$data), (NROW(Pf.1.mean$data) + 1):NCOL(cr.median.mad)]
